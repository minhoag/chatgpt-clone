...
import { checkEnvironment, getUserFromDb } from "@/lib/utils";

export default async function GetStartedButton() {
  const session = await getUser();

  if (!session?.user) {
    redirect(checkEnvironment().concat("/login"));
  }
  const user = await getUserFromDb(session.user.email);

  if (!user) {
    redirect(checkEnvironment().concat("/register"));
  }
  const conversations = user.conversations;

  if (conversations.length === 0)
    return redirect(checkEnvironment().concat("/chat"));
